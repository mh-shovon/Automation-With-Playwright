...
const { CustomTest } = require('./Pages/Test-Base')
...
CustomTest("User login and complete an order(Data send as Fixture)", async({ page, testDataForOrder })=>{
    //POM Controller Page
    const pomControllerPage = new PomControllerPage(page)
    ...
    const loginPage = pomControllerPage.getLoginPage();
    await loginPage.openLoginPage();
    await loginPage.loginWithValidCredentials(testDataForOrder.userEmail, testDataForOrder.userPassword);
    ...
    await checkoutPage.navigateToCheckoutPage();
    await page.waitForLoadState('networkidle');
    await checkoutPage.enterCardInfo(testDataForOrder.creditCardNumber, testDataForOrder.cvvCode, testDataForOrder.nameOnCard);
    await checkoutPage.enterCouponInfoAndVerify(testDataForOrder.couponCode);
    await checkoutPage.verifyingUserEmail(testDataForOrder.userEmail);
    await checkoutPage.countrySelection(testDataForOrder.desireCountryName);
    const orderId = await checkoutPage.submitOrderAndGetOrderId();
    console.log(orderId);
