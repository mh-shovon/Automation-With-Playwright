...
for(const data of dataSet){
    test(`Test-1 for :: ${data.productName}`, async({ page })=>{
        //POM Controller Page
    ...
    test('Test-2 :: Screenshot Capturing', async ({page})=>{
        await page.goto("https://rahulshettyacademy.com/AutomationPractice/");
        const inputBox = page.locator("#displayed-text");
        await expect(inputBox).toBeVisible();
        await page.screenshot({path:'./tests/Practice With Rahul Shetty/Section-16/Screenshots/Screenshot-1.png'}); //whole page
        await inputBox.screenshot({path:'./tests/Practice With Rahul Shetty/Section-16/Screenshots/Screenshot-2.jpg'}); //Specific element
        const hideBtn = page.locator("#hide-textbox");
        await hideBtn.click();
        await expect(inputBox).toBeHidden();
    });
    
    test('Test-3 :: Visual Comparison', async ({page})=>{
        await page.goto("https://flightexpert.com/");
        expect(await page.screenshot()).toMatchSnapshot('./tests/Practice With Rahul Shetty/Section-16/landing.png');
    });

    // Test file will run parallel 
    // But the tests are in the file will run sequentially
    // --grep @web --> it will run the test which are under the web tag.
